...
public:
    ...
    struct InputInfo {
        ...
        friend std::ostream &operator<<(std::ostream &stream, InputInfo const &info) {
            return stream << "Priority: " << info.priority << " Timeout: " << info.timeoutTime_ms;
        }
    };
    ...
    ///
    PriorityMuxer(unsigned ledCount);
...
private:
    ...
    std::map<int, InputInfo> _activeInputs;
