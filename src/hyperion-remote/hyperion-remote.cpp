...
int main(int argc, char * argv[])
{
    ...
    try
    {
        ...
        std::string defaultServerAddress = "localhost:19444";
        ...
        StringParameter    & argAddress    = parameters.add<StringParameter>   ('a', "address"   , Poco::format("Set the address of the hyperion server [default: %s]", defaultServerAddress).c_str());
        IntParameter       & argPriority   = parameters.add<IntParameter>      ('p', "priority"  , Poco::format("Use to the provided priority channel (the lower the number, the higher the priority) [default: %s]", defaultPriority).c_str());
        ...
        {
            std::string info = connection.getServerInfo();
