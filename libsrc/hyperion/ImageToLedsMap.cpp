    ...
    mColorsMap()
{
    ...
    for (const Led& led : leds)
    {
        ...
        unsigned minX_idx = xOffset + unsigned(floor(actualWidth  * led.minX_frac + 0.5));
        unsigned maxX_idx = xOffset + unsigned(floor(actualWidth  * led.maxX_frac + 0.5));
        unsigned minY_idx = yOffset + unsigned(floor(actualHeight * led.minY_frac + 0.5));
        unsigned maxY_idx = yOffset + unsigned(floor(actualHeight * led.maxY_frac + 0.5));
